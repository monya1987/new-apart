const parser = ($, record) => {
    ...
    // const buildingProgress = '.BuildingConstruction-items .BuildingConstruction-info';
    ...
    //     res.descr = cropSubStrings($(this).find('.placeholder').text().trim(), ['подтверждено документами', 'введено в экспл. в']);
    //     res.withDocs = $(this).find('.placeholder').text().includes('подтверждено документами');
    //     progress.push(res);
    // });
    //
    // const priceStat = [];
